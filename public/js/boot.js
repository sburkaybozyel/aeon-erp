      ...
      try {
        ...
        if (isLoginPage) {
          ...
          if (path.includes('staff.html') || path.includes('admin_mobile.html')) {
            ...
            const isMgrPage = path.includes('index.html');
            const isReceptionPage = path.includes('staff-reception.html');
            const isHkPage = path.includes('staff-housekeeping.html');
            const isRestaurantPage = path.includes('staff-restaurant.html');
            const isKitchenPage = path.includes('staff-kitchen.html');
            const isMaintenancePage = path.includes('staff-maintenance.html');
            
            let isCorrectPage = false;
            if (isMgrPage && ['admin', 'manager', 'yönetici', 'restoran müdürü'].includes(role)) isCorrectPage = true;
            else if (isReceptionPage && role === 'reception') isCorrectPage = true;
            else if (isHkPage && role === 'housekeeping') isCorrectPage = true;
            else if (isRestaurantPage && ['restaurant', 'waiter'].includes(role)) isCorrectPage = true;
