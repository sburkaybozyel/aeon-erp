...
const tenantQuery = () => `?tenant_id=${encodeURIComponent(window.tenantId || 'aeon')}`;
...
function connectEvents() {
  ...
  stream.addEventListener('request_created', event => {
    ...
    if (departments.some(department => String(department).toLocaleLowerCase('tr-TR') === 'bar')) notifyNewOrder(event);
  });
