...
export function renderLiveManagementLists(rooms, tables, inventory, catalog) {
  ...
  if (roomsBody) {
    ...
    rooms.forEach(r => {
      ...
      card.innerHTML = `<div><span class="admin-room-number">Oda ${esc(r.room_number)}</span><strong>${esc(r.room_type || 'Standart')} · ${Number(r.capacity || 0)} kişi</strong><small>${esc(r.floor || '-')} . kat · ${r.status === 'clean_vacant' ? 'Temiz ve hazır' : esc(r.status)}</small></div><div class="admin-room-management-actions">${occupied ? '<span class="admin-guest-tag">Konaklıyor</span>' : ''}<a class="admin-quick-link" href="/staff-reception.html?tenant_id=${encodeURIComponent(state.currentTenant || 'aeon')}">Ön büro</a>${occupied ? '' : `<button class="btn btn-danger btn-xs" onclick="deleteRoomLive('${r.id}')"><i class="fa-solid fa-trash-can"></i></button>`}</div>`;
  ...
  if (invBody) {
    ...
    invBody.querySelectorAll('.admin-inv-add-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (qty <= 0) return;
        try {
          const resp = await fetch('/api/inventory/add-stock?tenant_id=' + encodeURIComponent(window.__currentTenantId || 'aeon'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ inventory_id: invId, purchase_quantity: qty }) });
    ...
    invBody.querySelectorAll('.admin-inv-edit-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        modal.querySelector('#admin-inv-edit-form').addEventListener('submit', async (e) => {
          ...
          try {
            const resp = await fetch('/api/inventory/' + encodeURIComponent(invId) + '?tenant_id=' + encodeURIComponent(window.__currentTenantId || 'aeon'), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
            const data = await resp.json();
            if (!resp.ok) throw new Error(data.error);
            modal.remove();
