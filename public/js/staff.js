    ...
    if (Array.isArray(items)) {
      itemsListHtml = items.map(i => {
        ...
        return `<div style="padding: 6px 0; border-bottom: 1px dashed rgba(0,0,0,0.1); font-size:13px;">
          ...
          <strong style="color:var(--color-primary);">${i.quantity || 1}x</strong> ${nameClean}
        </div>`;
      }).join('');
    }
  } catch (e) {
    itemsListHtml = `<div style="padding: 6px 0; font-size:13px;">${order.details}</div>`;
  }
...
function formatStaffRequestDetails(details) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.map(item => `${item.quantity || 1}x ${item.name || item.itemId || 'Ürün'}`).join(', ');
    }
  } catch (e) {}
...
function renderStaffTechPanel(rooms, requests = []) {
  ...
  technicalRequests.forEach(request => {
    ...
    card.innerHTML = `
      ...
      <div class="text-sm" style="color:var(--color-text-main); line-height:1.5; margin-bottom:16px;">${request.details || 'Teknik servis talebi'}</div>
...
window.toggleRoomAc = (roomId, currentlyBroken) => {
  const room = state.roomsList.find(r => r.id === roomId);
  if (!room) return;
  
  room.ac_status = currentlyBroken === 'true' ? 'working' : 'broken';
  
  // Re-render
  renderTechRoomsPanel(state.roomsList);
  renderStaffTechPanel(state.roomsList); // Update the tasks panel too
};
      ...
      if (restaurantTasks.length === 0) {
        ...
        restaurantTasks.forEach(task => {
          ...
          card.innerHTML = `
            ...
            <div class="text-xs text-muted" style="margin-top: 4px;">${task.details}</div>
            <div class="kds-actions margin-top-sm">
              <button class="btn btn-success btn-xs btn-block" onclick="staffCompleteTask('${task.id}')">Tamamlandı</button>
            </div>
          `;
