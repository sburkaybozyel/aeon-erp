...
function renderAuditLogs(logs) {
  ...
  logs.forEach(log => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="badge ${getLogActionBadgeClass(log.action)}" style="font-size:10px; padding:2px 6px;">${log.action}</span></td>
...
function renderActiveGuests(rooms) {
  ...
  activeGuests.forEach(room => {
    ...
    tr.innerHTML = `
      <td><strong>Oda ${room.room_number}</strong>${dnd}</td>
      <td>${guestName}</td>
      <td>${phone}</td>
      <td>${arrival}</td>
      <td>${departure}</td>
  ...
  if (tablesContainer) {
    ...
    if (activeOrders.length > 0) {
      ...
      activeOrders.forEach(o => {
        ...
        div.innerHTML = `
          <span><strong>${o.target_identifier}</strong>: ${itemsList}</span>
          <span style="text-transform:uppercase; font-weight:bold; font-size:8px; color:var(--color-warning);">${o.status}</span>
        `;
        tablesContainer.appendChild(div);
      });
    }
  ...
  stockLogs.forEach(log => {
    ...
    div.innerHTML = `
      <div class="flex-between">
        ...
        <span class="text-muted" style="font-size:9px;">${log.staff_name}</span>
      </div>
      <div style="font-size:10px; margin-top:2px;">${log.details}</div>
    `;
...
function renderLiveTrackerAudit(logs) {
  ...
  recentLogs.forEach(log => {
    ...
    div.innerHTML = `
      ...
      <div style="font-size:10px; color:rgba(255,255,255,0.85); margin-top:2px;">${log.action}: ${log.details}</div>
    `;
    container.appendChild(div);
  });
}
