...
async function sendStaffPush(tenantId, payload, targetRoles = [], targetDepartments = [], targetStaffIds = []) {
  ...
  try {
    const db = await getDb(tenantId || 'restaurant_kitchen');
...
eventBus.on('precheckin_submitted', data => {
  const tenantId = data.tenantId || 'restaurant_kitchen';
...
eventBus.on('request_created', data => {
  broadcastSSE(data.tenantId || 'restaurant_kitchen', 'request_created', data);
});
eventBus.on('request_updated', data => {
  broadcastSSE(data.tenantId || 'restaurant_kitchen', 'request_updated', data);
});
...
export function registerOperationsRoutes(app) {
  ...
  app.get('/api/events', (req, res) => {
    ...
    const tenantId = req.tenantId || 'restaurant_kitchen';
