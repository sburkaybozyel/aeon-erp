...
export function configuredTenantForHost(host) {
  ...
  return process.env.MODULE_DEFAULT_TENANT || 'restaurant_kitchen';
}
...
function allowedTenants() {
  const configured = String(process.env.MODULE_ALLOWED_TENANTS || 'restaurant_kitchen')
    .split(',')
...
function staffPinKey() {
  const material = process.env.STAFF_PIN_ENCRYPTION_KEY || process.env.FIREBASE_PRIVATE_KEY || 'restaurant-kitchen-local-staff-pin-key';
