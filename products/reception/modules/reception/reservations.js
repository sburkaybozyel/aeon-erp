...
export function registerReservationRoutes({ app, eventBus }) {
  ...
  app.post('/api/reception/reservations', async (req, res) => {
    try {
      ...
      // validateReservationMoney only checks deposit_amount/nightly_rate; tax_amount,
      // discount_amount and a caller-supplied total_amount were previously used unvalidated
      // below (Number(data.tax_amount || 0), etc.) — a non-numeric or negative value (or a
      // typo'd string) would silently become NaN or a negative folio total once the
      // reservation/folio math runs downstream.
      for (const [field, label] of [['tax_amount', 'Vergi tutarı'], ['discount_amount', 'İndirim tutarı'], ['total_amount', 'Toplam tutar']]) {
        if (data[field] === undefined) continue;
        const value = Number(data[field]);
        if (!Number.isFinite(value) || value < 0) return res.status(400).json({ error: `${label} geçerli ve negatif olmayan bir sayı olmalıdır.` });
      }
