...
export function registerGuestPortalRoutes({ app, eventBus }) {
  ...
  app.post('/api/guest/precheckin/:token', async (req, res) => {
    ...
    await eventBus.emit('precheckin_submitted', { tenantId: req.tenantId, id: entryId, reservationId: reservation.id, reservationNumber: reservation.reservation_number, guestName: `${payload.first_name} ${payload.last_name}`, guest: payload, source: 'reservation_qr' });
  ...
  app.post('/api/guest/precheckin', async (req, res) => {
    ...
    await eventBus.emit('precheckin_submitted', { tenantId: req.tenantId, id: submissionId, guestName: `${payload.first_name} ${payload.last_name}`, guest: payload, source: 'fixed_qr' });
