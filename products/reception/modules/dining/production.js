...
// Resolves which kitchen station a food item's ticket should be fired to when it has no
// explicit menu_kitchen_profiles row. Older code here matched only literal Turkish station
// names (e.g. stations.find(s => s.name === 'Soğuk & Meze')); the seeded demo catalog now
// creates English-named stations ('Cold & Appetizers', 'Hot Kitchen', 'Grill', 'Pastry'), so
// those lookups always resolved to undefined and the caller's `if (!stationId) continue`
// silently dropped the kitchen ticket line entirely — the guest's food order would be paid
// for and confirmed, but the kitchen would never see it. This resolver tries both Turkish and
// English station names and always falls back to a real station id, so a ticket is never
// silently dropped regardless of which language the catalog/station names are in.
export function resolveFoodStationId(stations, item, profile) {
  if (profile?.station_id) return profile.station_id;
  if (!stations.length) return null;
  const byName = Object.fromEntries(stations.map(station => [String(station.name).toLocaleLowerCase('tr-TR'), station.id]));
  const defaultStation = byName['sıcak'] || byName['hot kitchen'] || stations[0].id;
  const name = String(item?.name || '').toLocaleLowerCase('tr-TR');
  if (/(meze|humus|cacık|salata|soğuk|salad|appetizer|starter)/.test(name)) return byName['soğuk & meze'] || byName['cold & appetizers'] || defaultStation;
  if (/(ızgara|çipura|levrek|ahtapot|grill|steak|burger|kebap)/.test(name)) return byName['ızgara'] || byName['grill'] || defaultStation;
  if (/(tatlı|baklava|dondurma|dessert|cake|pastry|tiramisu|cheesecake)/.test(name)) return byName['tatlı'] || byName['pastry'] || defaultStation;
  ...
  async function backfillKitchenTickets(db) {
    ...
    for (const order of openOrders) {
      ...
      for (const line of details) {
        ...
        const stationId = resolveFoodStationId(stations, item, profile);
