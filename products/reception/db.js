...
async function initializeD1Db(tenantId) {
  const db = new D1Db(globalThis.__MODULE_D1, tenantId);
  let imported = null;
  try {
    imported = await db.get("SELECT value FROM config WHERE key = 'cloudflare_d1_imported'");
  } catch (error) {
    if (process.env.MODULE_D1_BOOTSTRAP !== 'true') throw error;
  }
  if (!imported && process.env.MODULE_D1_BOOTSTRAP === 'true') {
...
async function initializeDb(tenantId) {
  ...
  db.suspendSave = true;
  await runMigrations(db, tenantId);
  if (tenantId === 'aeon' || tenantId === (process.env.MODULE_DEFAULT_TENANT || 'reception')) {
    await ensureDefaultRoomInventory(db);
    await ensureDefaultDiningMasterData(db);
    await ensureDefaultKitchenMasterData(db);
    await ensureDefaultMarinaMasterData(db);
