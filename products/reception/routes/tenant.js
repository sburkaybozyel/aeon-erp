...
export function registerTenantRoutes(app) {
  app.get('/api/tenant/branding', (req, res) => {
    let tenantId = req.tenantId || req.query.tenant_id || 'reception';
    if (Array.isArray(tenantId)) {
      tenantId = tenantId[0];
    }
    if (tenantId === 'default') tenantId = 'aeon';

    if (tenantId === (process.env.MODULE_DEFAULT_TENANT || 'reception')) {
      return res.json({
        tenant_id: tenantId,
        name: process.env.MODULE_NAME || 'AEON Reception',
  ...
  app.get('/api/system/health', async (req, res) => {
    try {
      const roomCount = await req.db.get('SELECT COUNT(*) AS cnt FROM rooms');
      res.json({
        ok: true,
        ...
        databases: {
          primary: { durable: hasD1Persistence() || hasDurablePersistence(), rooms: Number(roomCount?.cnt || 0) }
        },
        bridges: {
