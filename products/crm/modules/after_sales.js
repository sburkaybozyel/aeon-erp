...
async function receptionGet(db, path) {
  if (path !== '/reservations') throw new Error('Reception modülünde desteklenmeyen sorgu.');
  return db.all("SELECT reception_reservation_id AS id, reception_reservation_id AS erp_reservation_id, reservation_number AS reservation_no, source_id, status, check_in, check_out, total_amount, currency FROM reception_reservations ORDER BY check_in DESC");
}
...
export function initAfterSales({ app, getCrmDb, broadcastSSE, isManagement, requireManagement }) {
  ...
  // ── TAKİP KUYRUĞU ────────────────────────────────────────────────────
  // Reception'da çıkış yapmış (checked_out) rezervasyonları çekip, izinli kontaklar için
  ...
  app.post('/api/crm/followups/generate', async (req, res) => {
    try {
      ...
      let receptionReservations = [];
      try {
        receptionReservations = await receptionGet(db, '/reservations');
      } catch (e) {
        return res.status(502).json({ error: `Resepsiyon kayıtlarına ulaşılamadı: ${e.message}` });
      }
      let created = 0, skipped = 0;
      for (const reception of receptionReservations) {
        if (!reception.source_id) continue;
        const opp = await db.get("SELECT * FROM opportunities WHERE id = ?", [reception.source_id]);
        if (!opp) continue;

        // Satış sonrası: Reception'da çıkış yapmış rezervasyonlar
        if (reception.status === 'checked_out' || reception.status === 'in_house') {
          ...
          await db.run(
            ...
            [uid('fup'), opp.id, contact.id, opp.firm_id || null, reception.reservation_no, iso(1), `Satış sonrası memnuniyet araması — ${contact.first_name} ${contact.last_name}`, opp.owner_id || null]);
          created++;
        }

        // Konaklama öncesi: check-in'e 3 gün kalan rezervasyonlar
        if (reception.status === 'inquiry' || reception.status === 'option' || reception.status === 'confirmed') {
          if (!reception.check_in) continue;
          const daysLeft = Math.round((new Date(reception.check_in) - Date.now()) / 86400000);
          if (daysLeft >= 0 && daysLeft <= 3) {
            ...
            await db.run(
              ...
              [uid('fup'), opp.id, opp.contact_id || null, opp.firm_id || null, reception.reservation_no, reception.check_in, `Check-in öncesi takip — ${reception.reservation_no}`, opp.owner_id || null]);
            created++;
          }
        }
      }
