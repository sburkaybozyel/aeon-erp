(() => {
  ...
  const views = {
    ...
    integration: { title: 'Entegrasyon', subtitle: 'Resepsiyon senkronizasyon günlüğü', render: renderIntegration },
    'reception-reservations': { title: 'Resepsiyon', subtitle: 'Bağımsız resepsiyon portalından gelen misafir ve konaklama kayıtları', render: renderReceptionReservations },
          ...
          return `<div class="kanban-col">
            ...
            ${cards.map(o => `
              <div class="kanban-card" data-opp-id="${o.id}">
                ...
                ${o.erp_reservation_no ? `<div class="meta erp-badge" style="margin-top:4px"><span class="badge" style="background:#e0e7ff;color:#3730a3">Resepsiyon ${esc(o.erp_reservation_no)}</span></div>` : ''}
              </div>
  ...
  async function renderFollowups() {
    ...
    $('#view-container').innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; flex-wrap:wrap; gap:8px;">
        <div class="muted small">Resepsiyona senkronize rezervasyonlardan otomatik üretilir. İletişim izni olmayan misafirler kuyruğa düşmez.</div>
        <button class="btn primary" data-action="generate-followups">⟳ Resepsiyondan Takip Üret</button>
      </div>
      <div class="card">
        <table class="table">
          <thead><tr><th>Tip</th><th>Fırsat</th><th>Kontak</th><th>Rezervasyon</th><th>Son Tarih</th><th>Durum</th><th></th></tr></thead>
          <tbody>${fups.map(followupRow).join('') || '<tr><td colspan="7" class="empty">Takip yok — "Resepsiyondan Takip Üret" ile çıkış kayıtlarından kuyruk oluşturun.</td></tr>'}</tbody>
        </table>
      </div>
    `;
  ...
  async function renderReports() {
    ...
    $('#view-container').innerHTML = `
      ...
      <div class="grid cols-2" style="margin-top:16px">
        ...
        <div class="card">
          <div class="section-title">CRM ↔ Resepsiyon Gelir Mutabakatı</div>
          ${recon ? `
            <div class="stat-row"><span>CRM toplam</span><strong>${fmtAmount(recon.crm_total, 'EUR')}</strong></div>
            <div class="stat-row"><span>Resepsiyon toplam</span><strong>${fmtAmount(recon.erp_total, 'EUR')}</strong></div>
            ...
            <table class="table">
              <thead><tr><th>Fırsat</th><th>Resepsiyon No</th><th>Durum</th><th>CRM</th><th>Resepsiyon</th></tr></thead>
  ...
  async function renderIntegration() {
    let config = { reception_url: '—', reachable: false, pending_errors: 0 };
    let logs = [];
    ...
    $('#view-container').innerHTML = `
      <div style="display:flex; gap:16px; flex-wrap:wrap; margin-bottom:16px;">
        <div class="card" style="flex:1; min-width:280px;">
          <div class="section-title">Reception Bağlantısı</div>
          <div style="display:flex; align-items:center; gap:10px; margin:8px 0;">
            ...
            <span class="small muted">${esc(config.reception_url)}</span>
          </div>
          ${config.health ? `<div class="small muted">Reception: ${esc(config.health.system || '—')} · ${dateText(config.health.time)}</div>` : '<div class="small muted">Reception yanıt vermiyor — service binding ve CRM_MODULE_TOKEN kontrol edin.</div>'}
  ...
  async function renderReceptionReservations() {
    const rows = await api('/api/crm/reception-reservations');
    const events = await api('/api/crm/reception-events');
    $('#view-container').innerHTML = `
      <div class="card">
        <div style="display:flex; justify-content:space-between; align-items:center; gap:12px;">
          <div><div class="section-title" style="margin:0;">Resepsiyon aktarım günlüğü</div><div class="small muted">Rezervasyon, ön check-in ve check-in olayları bağımsız CRM'e otomatik düşer.</div></div>
          <button class="btn small ghost" data-action="refresh-reception">↻ Yenile</button>
        </div>
        <table class="table" style="margin-top:14px;"><thead><tr><th>Rezervasyon</th><th>Misafir</th><th>Konaklama</th><th>Oda</th><th>Durum</th><th>Tutar</th></tr></thead>
        <tbody>${rows.map(row => `<tr><td><strong>${esc(row.reservation_number || row.reception_reservation_id)}</strong><div class="small muted">${esc(row.reception_reservation_id)}</div></td><td>${esc(`${row.first_name || ''} ${row.last_name || ''}`.trim() || '—')}<div class="small muted">${esc(row.email || row.phone || '—')}</div></td><td>${esc(row.check_in || '—')} → ${esc(row.check_out || '—')}</td><td>${esc(row.room_number || row.room_type || 'Atanmadı')}</td><td><span class="badge">${esc(row.status || '—')}</span></td><td>${fmtAmount(row.total_amount, row.currency)}</td></tr>`).join('') || '<tr><td colspan="6" class="empty">Henüz resepsiyon kaydı gelmedi</td></tr>'}</tbody></table>
      </div>
      <div class="card" style="margin-top:16px;"><div class="section-title">Tüm operasyon olayları</div><div class="small muted" style="margin-bottom:10px;">Ön check-in, oda değişimi, talep, folyo, ödeme, fatura ve çıkış akışı</div><table class="table"><thead><tr><th>Olay</th><th>Rezervasyon</th><th>Misafir</th><th>Tutar</th><th>Zaman</th></tr></thead><tbody>${events.map(event => `<tr><td><span class="badge">${esc(event.event_type)}</span></td><td>${esc(event.reservation_number || event.reception_reservation_id || '—')}</td><td>${esc(`${event.first_name || ''} ${event.last_name || ''}`.trim() || '—')}</td><td>${event.amount ? fmtAmount(event.amount, event.currency) : '—'}</td><td class="small">${dateText(event.occurred_at)}</td></tr>`).join('') || '<tr><td colspan="5" class="empty">Henüz olay gelmedi</td></tr>'}</tbody></table></div>`;
    $('[data-action=refresh-reception]').addEventListener('click', () => renderReceptionReservations());
  }
  ...
  async function openOpportunityDetail(id) {
    ...
    openModal(esc(opp.title), `
      ...
      <div class="section-title">Resepsiyon Rezervasyonu</div>
      <div id="erp-panel">
        ${opp.erp_reservation_no
          ? `<div class="small" style="margin-bottom:6px">
            ...
            <div style="display:flex; gap:8px; flex-wrap:wrap;">
              <button class="btn small" data-action="refresh-erp">↻ Resepsiyon Durumunu Yenile</button>
              <button class="btn small" data-action="sync-erp">↔ Resepsiyona Güncelle</button>
            </div>`
          : opp.pipeline_stage === 'won'
            ? `<div class="small muted" style="margin-bottom:6px">Bu kazanılmış fırsat henüz resepsiyon rezervasyonuna dönüştürülmedi.</div>
               <button class="btn small primary" data-action="convert-erp">⇄ Resepsiyon Rezervasyonuna Aktar</button>`
            : `<div class="small muted">Fırsat kazanıldığında resepsiyon rezervasyonuna aktarılabilir.</div>`}
      </div>
    ...
    $('#stage-select').addEventListener('change', async e => {
      ...
      if (opp.erp_reservation_id && (stage === 'lost' || stage === 'won')) {
        try { await api(`/api/crm/integration/opportunities/${id}/sync`, { method: 'POST', body: JSON.stringify({ status: stage === 'lost' ? 'cancelled' : 'confirmed' }) }); } catch (error) { toast(`Resepsiyon senkronu: ${error.message}`, true); }
      }
    ...
    if (convertBtn) convertBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        toast(r.duplicate ? `Zaten aktarılmış: ${r.reservation_no}` : `Resepsiyon rezervasyonu: ${r.reservation_no}`);
        openOpportunityDetail(id);
      } catch (e) {
        toast(e.message, true);
        convertBtn.disabled = false;
        convertBtn.textContent = '⇄ Resepsiyon Rezervasyonuna Aktar';
      }
    });
    ...
    if (refreshErp) refreshErp.addEventListener('click', async () => {
      ...
      try {
        ...
        toast(`Resepsiyon durumu: ${r.erp_status}`);
        openOpportunityDetail(id);
      } catch (e) {
        toast(e.message, true);
    ...
    const syncErp = $('[data-action=sync-erp]');
    if (syncErp) syncErp.addEventListener('click', async () => {
      syncErp.disabled = true;
      try {
        await api(`/api/crm/integration/opportunities/${id}/sync`, { method: 'POST' });
        toast('Resepsiyon rezervasyonu güncellendi');
        openOpportunityDetail(id);
      } catch (e) { toast(e.message, true); syncErp.disabled = false; }
    });
  }
  ...
  function isManagement() {
    return ['yönetici', 'manager', 'admin'].includes(String(state.user?.role || '').toLocaleLowerCase('tr-TR'));
  }
  ...
  async function init() {
    $('#login-form').addEventListener('submit', async (e) => {
      ...
      try {
        await api('/api/auth/login', { method: 'POST', body: JSON.stringify({ pin: $('#login-password').value }) });
        await enterApp();
      } catch (err) {
