...
export function registerStayRoutes({ app, eventBus }) {
  ...
  app.post('/api/reception/checkin', async (req, res) => {
    try {
      ...
      await req.db.transaction(async tx => {
        ...
        await tx.run("UPDATE rooms SET status = 'occupied', guest_name = ?, arrival_date = ?, departure_date = ?, board_type = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [`${guests[0].first_name} ${guests[0].last_name}`, reservation.arrival_date, reservation.departure_date, reservation.board_type, user.name, room.id], {
    ...
    } catch (error) { res.status(500).json({ error: 'Check-in tamamlanamadı.' }); }
  });
  ...
  app.post('/api/reception/stays/:id/move', async (req, res) => {
    try {
      ...
      await req.db.transaction(async tx => {
        ...
        await tx.run("UPDATE rooms SET status = 'occupied', guest_name = ?, arrival_date = ?, departure_date = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [oldRoom.guest_name, reservation.arrival_date, reservation.departure_date, user.name, target.id], {
        ...
        await tx.run('UPDATE stays SET room_id = ?, updated_at = CURRENT_TIMESTAMP, updated_by = ?, version = version + 1 WHERE id = ?', [target.id, user.name, stay.id], {
  ...
  app.post('/api/reception/stays/:id/checkout', async (req, res) => {
    try {
      ...
      const user = actor(req);
      await req.db.transaction(async tx => {
        await tx.run("UPDATE stays SET status = 'checked_out', checkout_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP, updated_by = ? WHERE id = ?", [user.name, stay.id], {
